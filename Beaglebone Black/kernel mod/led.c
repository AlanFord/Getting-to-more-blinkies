 ...
 * @author Alan Ford
 * @date   21 Mar 2019
 * @brief  A kernel module for controlling a simple LED that
 * is connected to a GPIO. It is threaded in order that it can flash the LED.
 * The sysfs entry appears at /sys/caf/gpio49
 * Derived from http://www.derekmollow.ie/
...
static unsigned int blinkPeriod = 2000;     ///< The blink period in ms
 ...
 *  case it is gpio49, which is automatically defined in the ebb_LED_init() function below
...
static int __init ebb_LED_init(void){
   ...
   sprintf(ledName, "led%d", gpioLED);      // Create the gpio name for /sys/ebb/led49
   ...
   ledOn = true;
   gpio_request(gpioLED, "sysfs");          // gpioLED is 49 default, request it
